...
-   **Using an API resource** - a facade to separate our app from our raw API, which also includes a caching mechanism.
...
-   **Architectural flaws and how to solve them** - how to handle complex dependency graphs and resource setup execution order constraints without our code becoming a mess. This will be split into two posts--[one explaining the API](http://localhost:8000/blog/api-resource), and one diving into the implementation.
