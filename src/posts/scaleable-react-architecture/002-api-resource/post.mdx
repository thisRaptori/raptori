...
This last point demonstrates one of the most powerful aspects of the resource pattern: we get to remove mental overhead from our application code. It also applies to using the API facade alongside other resources. In addition to components consuming the simplified API directly, other resources are able to do so.

This structure keeps the api:consumer relationship loosely coupled while making the code more cohesive at both an architectural and implementation level.

As we will see when we explore other resources, this is one of the advantages inherent in this structure--next, we will look at the idea of a **Telemetry** resource, which has similar properties to those we've seen with the API facade.
